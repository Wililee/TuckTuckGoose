class board{
    constructor(pieces){
        ...
        */
        ...
        nextCol(c){
            switch(c){
                case Colour.W : return Colour.B;
                case Colour.B : return Colour.R;
                case Colour.R : return Colour.Bl;
                case Colour.Bl : return Colour.Y;
                case Colour.Y : return Colour.G;
                case Colour.G : return Colour.W;
            }
        }

        previousCol(c){
            switch(c){
                case Colour.W : return Colour.G;
                case Colour.B : return Colour.W;
                case Colour.R : return Colour.B;
                case Colour.Bl : return Colour.R;
                case Colour.Y : return Colour.Bl;
                case Colour.G : return Colour.Y;
            }
        }
        ...
        validMove(p, dx){
            ...
            if (p.x + dx > 17 && p.c != p.colour){
                if (this.isPieceAtPosition(p.c, 17) && this.pieceAtPosition(p.c, dx).colour == p.c)
                    return false;
            }
